...
DISTANCE_THRESHOLD_PLANE = 0.02 # Define la distancia máxima que puede tener un punto.
                                # a un plano estimado para ser considerado un interior
MAX_ITER_PLANE = 300   # Define con qué frecuencia se muestrea y verifica un plano aleatorio.
N_POINTS_SAMPLED_PLANE = 3 # Define el número de puntos que se muestrean aleatoriamente para estimar un plano.
...
class BoxEstimator():
    ...
    def process_pcl(self, raw_pcl):
        ...
        box = self.get_box_pcl(plane_eq, plane_inliers) # TODO, verifique si hay una casilla razonable incluso
        if box is None:
            return 0,0,0 # No box
    ...
    def calibrate(self, raw_pcl):
        self.raw_pcl = raw_pcl
        if len(raw_pcl.points) < 100:
            print("No se puede calibrar, hay muy pocos puntos en la nube de puntos.")
            return False

        self.crop_plc()
        if len(self.roi_pcl.points) < 100:
            print("No se puede calibrar, hay muy pocos puntos en la nube de puntos de ROI")
            return False
    ...
    def create_rotation_matrix(self, vec_in, vec_target):
        # Cree una matriz de rotación que gire vec_in a vec_target
    ...
    def crop_plc(self):
        # Recortar sólo para el ROI
        raw_pcl = self.raw_pcl
        raw_pcl_np = np.asarray(raw_pcl.points)

        # Calcular distancias de puntos
        pcl_dist = np.sqrt(np.sum(np.square(raw_pcl_np), axis=1))

        # TODO, el ROI debería realizarse en otro lugar
        # (probablemente ya esté en RGBD según el cuadro delimitador especificado)
    ...
    def get_box_pcl(self, plane_eq, plane_inliers):
        ...
        # Agrupar los valores atípicos, el más grande en buenas condiciones debería ser la caja.
        # TODO, si el más grande no supera el umbral -> sin casilla
    ...
    def get_plane_inliers(self, plane_eq, pts, thresh=0.05):
        ...
        # Seleccionar índices donde la distancia es mayor que el umbral
