...
def qr_webcam():
    ...
    with dai.Device(pipeline) as device:
        video = device.getOutputQueue(name="video", maxSize=20, blocking=False)
        while True:
            videoFrame = video.get()
            det = cv2.QRCodeDetector()
            for barcode in decode(videoFrame.getCvFrame()):
            ...
            if len(consolidado_qr)>1:
                break
        consolidado_qr={"lpn":myData}
        consolidado_qr_json = json.dumps(consolidado_qr)
        #consolidado_final = {"Longitud_cm": [round(mediana_LONG * 100, 0)], "Altura_cm": [round(mediana_ALTU * 100, 0)],
        #                     "Ancho_cm": [round(mediana_ANCHO * 100, 0)]}
        #consolidado_qr = np.unique(consolidado_qr)
        cv2.imwrite('/Users/javilizama/Desktop/HD/cubiScan/imagenes' + '/qr'+'_'+str(myData)+'.jpg',
            videoFrame.getCvFrame())
            #if cv2.waitKey(1) == ord('q'):
            #    break
    return consolidado_qr
#ver = qr_webcam()

def main_dimensionador():
    import time
    import cv2
    import depthai as dai
    ...
    with dai.Device(pipeline) as device:
        ...
        while True:
            ...
            for m in range(0, len(consolo)):
                ...
                consolo.loc[m, 'Volumen'] = consolo.loc[m, 'Longitud'] * consolo.loc[m, 'Ancho'] * consolo.loc[
                    m, 'Altura']
            ...
            consolidado_final = {"largo": round(mediana_LONG * 100, 0), "ancho": round(mediana_ANCHO * 100, 0),
                                 "altura": round(mediana_ALTU * 100, 0)}
